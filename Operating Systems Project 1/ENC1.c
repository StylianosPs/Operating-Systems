...
#define	SEMKEY2	((key_t) 7892) /* ENC1, CHAN semaphore key */
#define	SEMKEY3	((key_t) 7893) /* CHAN, ENC1 semaphore key */
...
int main(int argc, char *argv[]){
    ...
    if(shared_stuff->process==1){ /*P1->P2----------------------------------------*/
        ...
        semop(sem_id, &enter, 1); /*P1-ENC1 DOWN*/
        ...
        semop(sem_id2, &leave, 1);/*ENC1-CHAN UP*/
        waitpid(pid2, NULL, 0);/*WAIT FOR CHAN*/

        /*RESEND MESSAGE*/
        if(shared_stuff->flag==0){
            semctl(sem_id2, 0, IPC_RMID);/*ENC1-CHAN RMV*/
        ...
        semop(sem_id3, &enter, 1); /*CHAN-ENC1 DOWN*/
        ...
        /*MEMORY COMPARE BETWEEN ENC2 AND ENC1 CHECKSUM*/ 
        if(memcmp(hash,shared_stuff->checksum,MD5_DIGEST_LENGTH)==0){/*SAME*/
            ...
            shared_stuff->flag=0;
            semop(sem_id3, &leave, 1); /*CHAN-ENC1 UP*/
            exit(0);
        }
        ...
        semctl(sem_id3, 0, IPC_RMID);/*CHAN-ENC1 RMV*/
